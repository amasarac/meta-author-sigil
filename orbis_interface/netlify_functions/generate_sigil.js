  ...
  const eyeStroke = `hsl(${(hue + 180) % 360}, 85%, 75%)`;
  const spiralWidth = 6 + (Math.abs(seed) % 10); // Vary stroke width from 6 to 15
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${Math.floor(size*0.35)}" viewBox="0 0 1200 420">
    ...
    <g transform="translate(600, 210)">
      <path d="M0,40 c 40,-20 80,-60 80,-120 c 0,-90 -120,-160 -240,-80 c -40,30 -60,90 -40,130 c 30,60 100,90 200,70 c 160,-30 180,-150 100,-240" fill="none" stroke="${spiralStroke}" stroke-width="${spiralWidth}" stroke-linecap="round" opacity="0.98"/>
